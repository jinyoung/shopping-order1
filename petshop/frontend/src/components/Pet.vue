<template>
    ...
    <v-card style="width:300px;" outlined>
        ...
        <v-card-text>
            ...
            <int offline label="Appearance" v-model="value.appearance"
                    :editMode="editMode" :key="editMode" @change="change"/>
            <int offline label="Energy" v-model="value.energy"
                    :editMode="editMode" :key="editMode" @change="change"/>
    ...
    export default {
        ...
        created(){
            ...
            if(typeof this.value === 'object') {
                ...
                if(!('appearance' in this.value)) {
                    this.value.appearance = null;
                }
                if(!('energy' in this.value)) {
                    this.value.energy = null;
                }
        ...
        methods: {
            ...
            async eat() {
                try {
                    if(!this.offline){
                        var temp = await axios.post(axios.fixUrl(this.value._links.eat.href))
                        for(var k in temp.data) {
                            this.value[k]=temp.data[k];
                        }
                    }
            ...
            async sleep() {
                try {
                    if(!this.offline){
                        var temp = await axios.post(axios.fixUrl(this.value._links.sleep.href))
                        for(var k in temp.data) {
                            this.value[k]=temp.data[k];
                        }
                    }
            ...
            async speak() {
                try {
                    if(!this.offline){
                        var temp = await axios.post(axios.fixUrl(this.value._links.speak.href))
                        for(var k in temp.data) {
                            this.value[k]=temp.data[k];
                        }
                    }
